...
static inline uint16_t degrees_to_q3_9_raw(float degrees) {
    float radians = degrees * (float)M_PI / 180.0f;
    int v = (int)lroundf(radians * 512.0f);

    if(v < -2048) v = -2048;
    if(v >  2047) v =  2047;

    return (uint16_t)((uint16_t)v & 0x0FFFu);
}
...
static void synthetic_attitude(float t, uint16_t *roll_raw, uint16_t *pitch_raw) {
    float roll_deg  = 22.0f * sinf(0.75f * t);
    float pitch_deg = 15.0f * sinf(0.45f * t + 1.10f);

    *roll_raw  = degrees_to_q3_9_raw(roll_deg);
    *pitch_raw = degrees_to_q3_9_raw(pitch_deg);
}

int main(void) {
    struct timespec sleep_time = { 0, 16000000L };  // about 60 Hz
    ...
    printf("Synthetic VGA demo running. Ctrl+C to stop.\n");
    ...
    while(g_running) {
        ...
        uint16_t roll_raw, pitch_raw;
        synthetic_attitude(t, &roll_raw, &pitch_raw);

        ahrs_display_render(roll_raw, pitch_raw);
